...
#+LATEX_CLASS: article
#+LATEX_CLASS_OPTIONS: [a4paper, 12pt, twoside, titlepage]
...
#+LATEX_HEADER: \usetikzlibrary{shapes,arrows,matrix,fit}
#+LATEX_HEADER: \usepackage{circuitikz}
#+LATEX_HEADER: \pagestyle{fancy}
#+LATEX_HEADER: \cfoot{}
#+LATEX_HEADER: \fancyhead{}
#+LATEX_HEADER: \fancyhead[RE,LO]{\bfseries \nouppercase{\leftmark}}
#+LATEX_HEADER: \fancyhead[LE,RO]{\bfseries \thepage}
#+LATEX_HEADER: \usepackage{pdfpages}
#+CITE_EXPORT: biblatex backend=biber,style=ieee
#+BIBLIOGRAPHY: hvN.bib
...
temperatura de $77.35$ K ($-195.8$ ºC) [cite:@Practicas2025;@Lemmon2026].
...
la facilidad de obtener comercialmente nitrógeno líquido [cite:@Knutson1969].

Precisamente, el objeto de la presente memoria es describir la experiencia
presentada en [cite:@Practicas2025], presentar y analizar sus resultados.
...
un proceso mediante los balances adecuados [cite:@Practicas2025].
...
de entalpía de vaporización del N$_2$ [cite:@Practicas2025].
...
N$_2$ igual a $199.176$ J g$^{-1}$ [cite:@Lemmon2026].
...
#+NAME: fig:circuito
#+CAPTION: Esquema del circuito empleado en la práctica de laboratorio $-\text{reproducido de}$ [cite:@Practicas2025]$-$. El montaje está conformado por una fuente de tensión conectada con un resistor, de un interruptor para abrir o cerrar el circuito y de un amperímetro conectado en serie y un voltímetro en paralelo.
\begin{figure}[h!]
\begin{center}
...
La figura [[fig:diagrama]] resume el procedimiento mediante un diagrama de flujo.

#+NAME: fig:diagrama
#+CAPTION: Diagrama de flujo que esquematiza el procedimiento de la experiencia. En rojo se representan el sistema objeto de estudio y los resultados a analizar. Los rombos simbolizan decisiones y los cuadrados acciones.
\begin{figure}[h!]
\begin{center}
...
criterios expuestos en [cite:@Olalla2025] y [cite:@TEB2025]. El ajuste de los
datos y su representación gráfica se ha realizado empleando el /software/ libre
/gnuplot/ [cite:@gnuplot54p2], mientras que para la redacción de la
presente memoria se ha hecho uso del modo de edición /org mode/ del editor
de texto /Emacs/ [cite:@orgmode952]. La generación de las figuras [[fig:circuito]]
y [[fig:diagrama]] ha sido realizada mediante la librería $\text{Ti}k\texk{Z}$ de
...
GitHub con enlace [[https://github.com/angruicam/Termodinamica]]. En particular,
...
[[tab:4540]] y [[tab:30250]] muestran los resultados medidos. En la cabecera de cada
...
primer lugar, la tabla [[tab:mv]] presenta los valores calculados para la masa total
...
incertidumbres de ambas magnitudes. En la figura [[fig:termogravimetria]]
...
experimental. Pueden extraerse directamente del archivo [[fit][/fit.log/]] generado
por /gnuplot/.
...
#+NAME: tab:4540
#+CAPTION: Series de datos medidos en el laboratorio para las corrientes de $0.45$ A y $0.4$ A. El subíndice se relaciona con las centésimas de amperio de la serie correspondiente. Los valores de las masas expuestos se corresponden con las medidas del granatario, y no directamente con la masa de nitrógeno líquido evaporada. Dichos valores se obtienen en la tabla [[tab:mv]].
...
#+NAME: tab:mv
#+CAPTION: Valores de la masa de nitrógeno líquido evaporado para cada intervalo de tiempo. El subíndice se relaciona con las centésimas de amperio de la serie correspondiente y el superíndice indica que es la masa de N$_2$ que se ha evaporado.
|---+-----------+---------------+---------------+---------------+---------------+---------------|
|   |    <c>    |      <c>      |      <c>      |      <c>      |      <c>      |      <c>      |
...
con la media de los valores tabulados en las tablas [[tab:4540]] y [[tab:30250]].
...
La tabla [[tab:hv]] muestra los valores de la tasa de evaporación, $\dot{m}$,
y la potencia disipada, $\dot{W}$, para cada experiencia.

#+NAME: tab:hv
#+CAPTION: Valores de la tasa de evaporación del N$_2$ y de la potencia disipada para cada una de las condiciones experimentales fijadas en la experiencia.
|---+------------------------+------------------------------+-----------------------|
|   |          <c>           |             <c>              |          <c>          |
...
como se expuso en la ecuación [[eq:balance]], en el balance energético solo es
...
ambiente no varían. Por tanto, los datos de la tabla [[tab:hv]] pueden ajustarse
linealmente a la ecuación:
#+NAME: eq:ajuste
\begin{equation}
\dot{m} = -\frac{1}{\Delta h} \dot{W} + \frac{\dot{Q}}{\Delta h}.
\end{equation}
De nuevo, pueden extraerse del archivo [[fith][/fit.log/]] generado por /gnuplot/ los
parámetros del ajuste con su respectivas incertidumbres. La figura [[fig:entalpia]]
...
#+NAME: fig:entalpia
#+CAPTION: Representación de los valores obtenidos para las tasas de evaporación, $\dot{m}$, frente a la potencia disipada, $\dot{W}$. Ambas magnitudes pueden relacionarse linealmente mediante la ecuación [[eq:ajuste]] de conservación de la energía. A partir del ajuste pueden estimarseo los valores del incremento de entalpía de vaporización del N$_2$ y de las pérdidas por unidad de tiempo. En rojo se representan las barras de error de los datos.
\begin{figure}[h!]
\begin{center}
...
a presión atmosférica ($199.176$ J g$^{-1}$) extraído de [cite:@Lemmon2026].
Si se analiza la figura [[fig:termogravimetria]] se observa que la serie de datos
...
Si se repite el ajuste de la figura [[fig:entalpia]] excluyendo el punto
